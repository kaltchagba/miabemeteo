...
async def get_alertes() -> AlertesResponse:
    ...
    return AlertesResponse(
        alertes=[AlerteMeteo(**a) for a in alertes_raw], nb_actives=len(alertes_raw)
    )
...
async def get_previsions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={"erreur": "Open-Meteo indisponible", "detail": str(e)},
        )

    return PrevisionsResponse(
        ville=ville, pays=pays, jours=[JourPrevision(**j) for j in donnees]
    )
...
async def export_meteo(
    ...
    for entree in historique:
        writer.writerow(
            [
                entree.get("timestamp", ""),
                ville,
                pays,
                entree.get("temperature_c", ""),
                entree.get("humidite_pct", ""),
                entree.get("vent_kmh", ""),
                entree.get("description", ""),
                "",
                "",
                "historique",
            ]
